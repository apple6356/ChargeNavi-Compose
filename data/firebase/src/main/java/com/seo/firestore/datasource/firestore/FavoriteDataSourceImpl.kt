...
class FavoriteDataSourceImpl {
     ...
     * 즐겨찾기 추가
     * */
    suspend fun create(data: Favorite): FireResult<Favorite> = runCatching {
     ...
     * 즐겨찾기 삭제
     * */
    suspend fun delete(userId: String, csId: String): FireResult<Boolean> = runCatching {
...
//        val resultTask = favoriteDocRef.get().await()
     ...
     * */
    suspend fun findByUserIdAndCsId(userId: String, csId: String): FireResult<Favorite> = runCatching {
        val resultTask = FirestoreCollectionFilter.getFavoriteFirestoreCollection()
